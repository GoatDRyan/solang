...
export function useToeflListeningAttempt(attemptId) {
  ...
  const refreshAttempt = useCallback(async () => {
    ...
    try {
      ...
      const listeningSection =
        data.sections.find((item) => item.section_key === 'listening') ||
        data.sections[0] ||
        null;

      setAttempt(data.attempt);
      setSection(listeningSection);
    } catch (error) {
      setAttemptError(error.message || 'Failed to load TOEFL Listening attempt.');
    } finally {
      setIsAttemptLoading(false);
    }
  }, [attemptId]);
