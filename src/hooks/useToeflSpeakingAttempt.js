...
export function useToeflSpeakingAttempt(attemptId) {
  ...
  const refreshAttempt = useCallback(async () => {
    ...
    try {
      ...
      const speakingSection =
        data.sections.find((item) => item.section_key === 'speaking') ||
        data.sections[0] ||
        null;

      setAttempt(data.attempt);
      setSection(speakingSection);
    } catch (error) {
      setAttemptError(error.message || 'Failed to load TOEFL Speaking attempt.');
    } finally {
      setIsAttemptLoading(false);
    }
  }, [attemptId]);
